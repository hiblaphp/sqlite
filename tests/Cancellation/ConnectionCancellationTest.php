...
describe('AsyncConnection - Cancellation', function () {
    ...
    it('tears down the connection and kills the worker process when an active prepared statement execution is cancelled with kill_worker_on_cancel enabled', function () {
        $config = dbConfig([
            'force_sync' => false,
            'kill_worker_on_cancel' => true,
        ]);
        ...
        try {
            $stmt = await($conn->prepare(slowCteQuery()));
            $execPromise = $conn->executeStatement($stmt, []);

            Loop::addTimer(0.1, function () use ($execPromise) {
                $execPromise->cancel();
            });
